 ...
 *
 */
public class AbstractButtonAdapter implements JsonDeserializer<AbstractButton>, JsonSerializer<AbstractButton> {

	@Override
	public AbstractButton deserialize(JsonElement json, Type type, JsonDeserializationContext context)
		...
		if(json != null){
			...
			//获取子菜单
			JsonElement sbElement = obj.get("sub_button");
			if(sbElement != null && sbElement.isJsonArray() && sbElement.getAsJsonArray().size()>0){
				SubMenu sb = new SubMenu(obj.get("name").getAsString());
				Button[] buttons = context.deserialize(sbElement, Button[].class);
				for(Button b : buttons){
					sb.getSubButton().add(b);
				}
				return sb;
			}
		}
